...
Symptoms: 	                    fièvre (≥ 38 ºC) 
                                ...
                                vomit tout ;
                                convulsions ou ATCD de convulsions liées à la présente symptomatologie ;
                                polypnée.
...
def query_missing_data(path: str, section: str):
    df = pd.read_csv(path)
    df_queries = pd.DataFrame(columns=['Code Echantillon', 'Nom de l\'investigateur', 'Disctrict sanitaire', 'Valeur actuelle', 'Erreurs/Confirmations', 'Actions', 'Section', 'Valeur souhaitée', 'Observations'])
