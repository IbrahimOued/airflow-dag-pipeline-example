    ...
    def bobo():
        ...
        def import_data():
            ...
            def read_bobo_csv_files():
                ...
                cols_to_drop = ['Event', 'Program stage', 'Program instance', 'Longitude', 'Latitude', 'Organisation unit code', 'Organisation unit', 'Tracked entity instance']
        ...
        def store_data():
            ...
            def convert_df_to_sql():
                ...
                df = pd.read_csv('/home/ibra/documents/afroscreen/clean_data/bobo/merged_dhis2_data.csv')

                numeric_columns = df.select_dtypes(include=['int64', 'float64']).columns
                df[numeric_columns] = df[numeric_columns].fillna(-1)
                categoric_columns = df.select_dtypes(include=['object']).columns
                df[categoric_columns] = df[categoric_columns].fillna('NULL')
    ...
    def ouaga():
        ...
        def import_data():
            ...
            def read_ouaga_csv_files():
                ...
                # list of the columns to drop
                cols_to_drop = ['Event', 'Program stage', 'Program instance', 'Longitude', 'Latitude', 'Organisation unit code', 'Organisation unit', 'Tracked entity instance', 'COVID Preciser Autres causes']
                
                raw_data_dir = '/home/ibra/documents/afroscreen/raw_data/ouaga'

                cols_section_1 = list(pd.read_csv(os.path.join(raw_data_dir, "1_carine.csv"), nrows=1))
                cols_section_2 = list(pd.read_csv(os.path.join(raw_data_dir, "2_carine.csv"), nrows=1))
                cols_section_3 = list(pd.read_csv(os.path.join(raw_data_dir, "3_carine.csv"), nrows=1))
                cols_section_4 = list(pd.read_csv(os.path.join(raw_data_dir, "4_carine.csv"), nrows=1))
                cols_section_5 = list(pd.read_csv(os.path.join(raw_data_dir, "5_carine.csv"), nrows=1))
                cols_section_6 = list(pd.read_csv(os.path.join(raw_data_dir, "6_carine.csv"), nrows=1))
                cols_section_7 = list(pd.read_csv(os.path.join(raw_data_dir, "7_carine.csv"), nrows=1))
                cols_section_8 = list(pd.read_csv(os.path.join(raw_data_dir, "8_carine.csv"), nrows=1))
                cols_section_9 = list(pd.read_csv(os.path.join(raw_data_dir, "9_carine.csv"), nrows=1))
                cols_section_10 = list(pd.read_csv(os.path.join(raw_data_dir, "10.csv"), nrows=1))
                # Read for each agent
                df_section_1 = pd.concat([pd.read_csv(os.path.join(raw_data_dir, "1_carine.csv"), usecols=[i for i in cols_section_1 if i not in cols_to_drop], converters=converter), pd.read_csv(os.path.join(raw_data_dir, "1_fatima.csv"), usecols=[i for i in cols_section_1 if i not in cols_to_drop], converters=converter)])
                df_section_2 = pd.concat([pd.read_csv(os.path.join(raw_data_dir, "2_carine.csv"), usecols=[i for i in cols_section_2 if i not in cols_to_drop], converters=converter), pd.read_csv(os.path.join(raw_data_dir, "2_fatima.csv"), usecols=[i for i in cols_section_2 if i not in cols_to_drop], converters=converter)])
                df_section_3 = pd.concat([pd.read_csv(os.path.join(raw_data_dir, "3_carine.csv"), usecols=[i for i in cols_section_3 if i not in cols_to_drop], converters=converter), pd.read_csv(os.path.join(raw_data_dir, "3_fatima.csv"), usecols=[i for i in cols_section_3 if i not in cols_to_drop], converters=converter)])
                df_section_4 = pd.concat([pd.read_csv(os.path.join(raw_data_dir, "4_carine.csv"), usecols=[i for i in cols_section_4 if i not in cols_to_drop], converters=converter), pd.read_csv(os.path.join(raw_data_dir, "4_fatima.csv"), usecols=[i for i in cols_section_4 if i not in cols_to_drop], converters=converter)])
                df_section_5 = pd.concat([pd.read_csv(os.path.join(raw_data_dir, "5_carine.csv"), usecols=[i for i in cols_section_5 if i not in cols_to_drop], converters=converter), pd.read_csv(os.path.join(raw_data_dir, "5_fatima.csv"), usecols=[i for i in cols_section_5 if i not in cols_to_drop], converters=converter)])
                df_section_6 = pd.concat([pd.read_csv(os.path.join(raw_data_dir, "6_carine.csv"), usecols=[i for i in cols_section_6 if i not in cols_to_drop], converters=converter), pd.read_csv(os.path.join(raw_data_dir, "6_fatima.csv"), usecols=[i for i in cols_section_6 if i not in cols_to_drop], converters=converter)])
                lnrg_cols = {
                    "COVID Date de la PCR": "COVID Date de la PCR LNRG",
                    "COVID PCR realisee": "COVID PCR realisee LNRG",
                    "N° Colis SONAPOST": "N° Colis SONAPOST LNRG",
                ...
                df_section_6.rename(lnrg_cols, inplace=True) 
                df_section_7 = pd.concat([pd.read_csv(os.path.join(raw_data_dir, "7_carine.csv"), usecols=[i for i in cols_section_7 if i not in cols_to_drop], converters=converter), pd.read_csv(os.path.join(raw_data_dir, "7_fatima.csv"), usecols =[i for i in cols_section_7 if i not in cols_to_drop], converters=converter)])
                df_section_8 = pd.concat([pd.read_csv(os.path.join(raw_data_dir, "8_carine.csv"), usecols=[i for i in cols_section_8 if i not in cols_to_drop], converters=converter), pd.read_csv(os.path.join(raw_data_dir, "8_fatima.csv"), usecols =[i for i in cols_section_8 if i not in cols_to_drop], converters=converter)])
                sec8_cols = {
                    "COVID Temperature": "COVID Temperature Investigation"
                }
                df_section_8.rename(sec8_cols, inplace=True)
                df_section_9 = pd.concat([pd.read_csv(os.path.join(raw_data_dir, "9_carine.csv"), usecols =[i for i in cols_section_9 if i not in cols_to_drop], converters=converter), pd.read_csv(os.path.join(raw_data_dir, "9_fatima.csv"), usecols =[i for i in cols_section_9 if i not in cols_to_drop], converters=converter)])
                df_section_10 = pd.read_csv(os.path.join(raw_data_dir, "10.csv"), usecols=[i for i in cols_section_10 if i not in cols_to_drop], converters=converter)
